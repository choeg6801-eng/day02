...
st.title(" 📋미니 선호도 조사")
st.caption("위젯을 조작하려면 화면 아래 '실시간 응답 요약'이 바로 바뀝니다.")
...
review= st.checkbox("6)강의 내용에 만족하시나요?", key="widget_review")

#7번 슬라이더 만족 점수 1~5
score=st.slider("7)이 강의 만족도 점수(1~5점)",min_value=1,max_value=5,value=1, key= "widget_score")


#8 텍스트 영역: 여러줄의 입력이 필요할 때 자유 의견 입력 
feedback = st.text_area("8)자유롭게 의견을 남겨주세요", key="widget_feedback")

# 9버튼: 클릭 여부(true/false)를 반환한다. 클릭했을 때만 아래 코드가 실행된다 
submitted = st.button("✅제출하기",key="widget_submit_btn")

st.markdown("----")
st.subheader(" 실시간 응답 요약")

# 위젯 값들을 버튼을 누르지 않아도 조작하는지 즉시 바로 갱신된다. 
...
if submitted : 
    ...
    st.write(f"- 강의 만족도 여부: **{'만족' if review else '미체크'}**/만족도 점수: **{score}점**")
    st.write(f"- 자유 의견 :**{feedback if feedback else '(작성안함)'}**")
             
else:
     st.write("위에 항목을 입력한 뒤 제출하기버튼을 눌러주세요") 